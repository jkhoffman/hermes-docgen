...
import { MarkdownOptions, formatMarkdown, generateFilePath, writeFile } from "../markdown";
import { DocItem, extractDocumentation as extractFromSourceFile } from "../parser/traversal";
import { ParserError, TypeScriptParser } from "../parser";
...
export class DocumentationGenerator {
	...
	public async generate(
		...
		for (const [fileName, items] of fileGroups.entries()) {
			...
			const writeResult = await writeFile(outputPath, markdown, { overwrite: true });
		...
		const indexResult = await writeFile(
			indexPath,
			indexItems,
			{ overwrite: true }
		);
